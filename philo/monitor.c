...
/*   Updated: 2025/07/13 01:40:31 by malbayra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*monitor_dinner(void *data)
{
	...
	while (!all_threads_running(&table->table_mutex,
		...
		;
	while (!simulations_fnished(table) && !is_error(table))  // Hata durumunu kontrol et
	{
		i = -1;
		while (++i < table->philo_num && !simulations_fnished(table) && !is_error(table))
		{
